...
{
    public class UpdateCultureObjectValidator:AbstractValidator<UpdateCultureObjectDto>
    {
        public UpdateCultureObjectValidator(ICultureObjectService cultureObjectService,IUserService responsiblePersonService)
        {
            RuleFor(x => x.Id).MustAsync(cultureObjectService.CultureObjectExists).WithMessage(x => $"Culture object with id '{x.Id}' does not exist");
